<x-app-layout>
                ...
                <x-flash-message status="session('status')" />
                  <form method="post" action="{{ route('owner.products.update', ['product' => $product->id])}}">
                    ...
                    @method('put')
                      <div class="-m-2">
                        ...
                        <div class="p-2 w-full flex justify-around mt-4 mx-auto">
                  ...
                  <form id="delete_{{$product->id}}" method="post" action="{{ route('owner.products.destroy', ['product' => $product->id])}}">
                    ...
                    <div class="p-2 w-full flex justify-around mt-32">
                      <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-black bg-red-500 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">削除する</a>
                    </div>
                  </form>
              </div>
          </div>
  ...
  <script>
    'use strict'//厳しめにチェックする
      const images = document.querySelectorAll('.image') //全てのimageタグを取得。querySelectorAllで引数にある全てのクラスを取得
